...
<div class="row">
    <div class="col-sm-12">
        <div class="nest">
            <div class="title-alt"><h6>Ubah Transaksi</h6></div>
                    ...
                    <div class="row">
                        <div class="col-sm-6">
                            <div class="form-group">
                                ...
                                <div class="col-lg-8">
                                    <input type="text" class="form-control" name="no_transaksi" id="no_transaksi" value="{{$data->no_transaksi}}" placeholder="No Transaksi">
                                </div>
                            </div>
                            ...
                            <div class="form-group">
                                ...
                                <div class="col-lg-8">
                                    <input type="text" class="form-control" name="pembeli" id="pembeli" value="{{$data->pembeli}}" placeholder="Pembeli">
                                </div>
                            </div>
                        </div>
                        <div class="col-sm-6">
                            <div class="form-group">
                                ...
                                <div class="col-lg-8">
                                    <input type="text" class="form-control datepicker" name="tgl" id="tgl" value="{{$data->tgl}}" placeholder="Tanggal Transaksi" readonly>
                                </div>
                            </div>
                            <div class="form-group">
                                <label class="col-lg-4 col-sm-4 control-label">Total Bayar</label>
                                <div class="col-lg-8">
                                    <input type="text" class="form-control" name="total_bayar" id="total_bayar" value="Rp. {{Custom::conver_rupiah($data->total_bayar)}}" placeholder="Total Bayar" readonly>
                                </div>
                            </div>
                        </div>
                    </div>
...
function load_bantu(id) {
    $.ajax({
        ...
        success	:function(data) {
            var data = data.data.transaksi_bantu;
            var transaksi_bantu = "";
            $.each(data, function (key, item)  {
                load_produk(item.produk, item.qty);
            });
            out_load();
        },
...
$('#data_form').on('submit', function(event) {
    ...
    $.ajax({
        type	: "POST",
        dataType: "json",
        url		: "{{asset('transaksi/update/'.$data->id)}}",
        data	: idata,
        processData: false,
        contentType: false,
        cache 	: false,
